...
class LumiChatTool {
  ...
  setupToolHandlers() {
            ...
            inputSchema: {
              ...
              properties: {
                ...
                replyPreview: {
                  type: "string",
                  description:
                    "Optional: Preview text of message being replied to",
                },
              },
              required: ["message"],
            },
            ...
            inputSchema: {
              ...
              properties: {
                ...
                includeReplies: {
                  type: "boolean",
                  description:
                    "Include replies to Lumi messages (default: true)",
                  default: true,
                },
              },
              required: [],
    ...
    this.server.setRequestHandler(CallToolRequestSchema, async (request) => {
      ...
      // Always work with an object to avoid `TypeError: Cannot destructure property ... of 'undefined'`
      const args = rawArgs ?? {};
        ...
        ? messages
            .map((msg) => {
              const replyInfo = msg.reply_to_id
                ...
                : "";
              return `[${msg.id}] ${msg.username}${replyInfo}: ${
                msg.message
  ...
  async checkLumiMentions(args) {
    ...
    const mentions = messages.filter((msg) => {
      ...
      const isReplyToLumi =
        includeReplies && msg.reply_to_username === LUMI_USERNAME;
    ...
    const mentionText =
      mentions.length > 0
        ? mentions
            .map((msg) => {
              const type = msg.message.toLowerCase().includes("@lumi")
                ? "MENTION"
                : "REPLY";
              const replyInfo = msg.reply_to_id
                ? ` (replying to ${msg.reply_to_username})`
                : "";
              return `[${type}] [${msg.id}] ${msg.username}${replyInfo}: ${
                msg.message
              } (${new Date(msg.created_at).toLocaleTimeString()})`;
            })
            .join("\n")
  ...
  async getOnlineUsers(args) {
    ...
    if (!response.ok) {
      throw new Error(
        `Failed to get online users: ${response.status} ${response.statusText}`
      );
    }
      ...
      users.length > 0
        ? users
            .map((user) => {
              ...
              return `• ${user.username}${typingStatus} (last seen: ${new Date(
                user.last_seen
              ).toLocaleTimeString()})`;
            })
            .join("\n")
